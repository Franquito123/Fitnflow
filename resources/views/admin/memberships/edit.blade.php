            ...
            <div class="card shadow-sm" style="border: 2px solid #1A365D;">
                <div class="card-header py-2" style="background-color: #1A365D; color: #FFFFFF; border-bottom: 3px solid #FF6B35;">
                    <h2 class="mb-0" style="font-weight: 700; font-size: 1.8rem;">
                        <i class="fas fa-id-card-alt me-2"></i> EDITAR MEMBRESÍA
                    </h2>
                </div>

                <div class="card-body p-3">
                    <form method="POST" action="{{ route('admin.memberships.update', $membership) }}" class="needs-validation" novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            @enderror
                            <small class="text-muted d-block mt-1" id="nameCharCount">{{ strlen(preg_replace('/[^a-zA-ZáéíóúÁÉÍÓÚñÑ\s]/', '', old('name', $membership->name))) }} / 50 caracteres</small>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" id="description" rows="3"
                                ...
                                style="border: 2px solid #1A365D; border-radius: 6px; padding: 10px 14px; font-size: 1.3rem;"
                                placeholder="Ingrese la descripción de la membresía">{{ old('description', $membership->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback" style="font-size: 1.0rem;">
                                    <i class="fas fa-exclamation-circle me-1"></i>{{ $message }}
                                </div>
                            @enderror
                            <small class="text-muted d-block mt-1" id="descCharCount">{{ strlen(old('description', $membership->description)) }} / 200 caracteres</small>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status_id" id="status_id" 
                                ...
                                @foreach($statuses as $status)
                                    <option value="{{ $status->id }}" {{ old('status_id', $membership->status_id) == $status->id ? 'selected' : '' }}>
                                        {{ $status->name }}
                                    </option>
                                @endforeach
                            </select>
